  ...
  var getFileInfo = function(file) {
    ...
    $.getJSON(fileConnector + '?mode=getinfo&path=' + encodeURIComponent(file) + '&time=' + d.getMilliseconds(), function(data) {
      if(data['Code'] == 0) {
        ...
        $('#fileinfo').find('img').replaceWith(data['Preview'].replace('{WIDTH2}', '32'));
      ...
      if(data) {
        ...
        if($('#fileinfo').data('view') == 'grid') {
          ...
          for(key in data) {
            ...
            result += '<li class="' + cap_classes + '"' + title + '"><div class="clip" data-path="' + data[key]['Path'] + '" >';

            if(data[key]['Preview'].charAt(0) == '<') {
              result += data[key]['Preview'].replace('{WIDTH2}', scaledWidth / 2);
            } else {
              if(data[key]['Preview'].indexOf('fa fa-') == 0) {
                result += '<i class="' + data[key]['Preview'] + '" style="font-size:' + scaledWidth + 'px" alt="' + data[key]['Path'] + '"  ></i>';
              } else {
                result += '<img src="' + data[key]['Preview'] + '" width="' + scaledWidth + '" alt="' + data[key]['Path'] + '"  />';
              }
            }
            result += '</div><p>' + data[key]['Filename'] + '</p>';
