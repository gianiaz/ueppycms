...
class Filemanager {
  ...
  private function get_file_info($path = '', $thumbnail = false) {
    ...
    if(is_dir($current_path)) {
      ...
      switch(strtolower($this->item['filetype'])) {
        case 'txt':
          $this->item['preview'] = 'fa fa-file-text-o';
          break;
        case 'xml':
        case 'xsl':
          $this->item['preview'] = 'fa fa-file-excel-o';
          break;
        default:
          $this->item['preview'] = '<span class="fa-stack" style="font-size:{WIDTH2}px"><span class="fa-stack-1x filetype-text">'.$this->item['filetype'].'</span><i class="fa fa-file-o fa-stack-2x"></i></span>';
          break;
