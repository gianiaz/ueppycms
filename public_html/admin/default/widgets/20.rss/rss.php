...
if(!function_exists('rssWidget')) {
  function rssWidget($debug = 0) {
    ...
    $return['size']   = 4;
    $return['conta']  = 0;
    $return['title']  = Traduzioni::getLang('rss', 'TITLE_WIDGET');

    if(defined('SET_ZRSSFEED') && filter_var(SET_ZRSSFEED, FILTER_VALIDATE_URL)) {

      $GLOBALS['pathJS'][] = DOC_ROOT.REL_ROOT.'bower_components/jquery.scrollTo/jquery.scrollTo.min.js';

      $cacheFile = DOC_ROOT.REL_ROOT.UPLOAD.'cache/feed-ueppycms-admin.xml';

      if(!file_exists($cacheFile) || filemtime($cacheFile) < time() - $cacheLifeTime) {

        $ch = curl_init();
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($ch, CURLOPT_URL, SET_ZRSSFEED);
